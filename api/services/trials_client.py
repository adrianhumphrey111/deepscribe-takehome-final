...
class ClinicalTrialsClient:
    ...
    async def search_trials(self, patient_data: PatientData, max_results: int = 25) -> List[Trial]:
        ...
        try:
            ...
            logger.info(f"Found {len(trials)} trials before LLM eligibility analysis")
    ...
    async def _build_search_params(self, patient_data: PatientData, max_results: int) -> Dict[str, Any]:
        ...
        # Age filter with 2-year buffer - temporarily disabled due to API syntax issues
        # TODO: Re-enable once correct ClinicalTrials.gov age filter syntax is confirmed
        # if patient_data.age:
        #     min_age = max(18, patient_data.age - 2)  # 2-year buffer below, minimum 18
        #     max_age = patient_data.age + 2  # 2-year buffer above
        #     age_filter = f"AREA[MinimumAge]RANGE[MIN, {max_age} years] AND AREA[MaximumAge]RANGE[{min_age} years, MAX]"
